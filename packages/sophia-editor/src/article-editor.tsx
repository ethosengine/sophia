...
import * as SophiaLinter from "@ethosengine/sophia-linter";
...
export default class ArticleEditor extends React.Component<Props, State> {
    ...
    // Maps to store refs by index (replaces string refs)
    private editorRefs = new Map<number, Editor | null>();
    private frameRefs = new Map<number | "all", IframeContentRenderer | null>();

    // eslint-disable-next-line react/sort-comp
    private setEditorRef = (index: number, ref: Editor | null) => {
        if (ref) {
            this.editorRefs.set(index, ref);
        } else {
            this.editorRefs.delete(index);
        }
    };

    private setFrameRef = (
        key: number | "all",
        ref: IframeContentRenderer | null,
    ) => {
        if (ref) {
            this.frameRefs.set(key, ref);
        } else {
    ...
    _updateIssues() {
        ...
        sections.forEach((section) => {
            ...
            const sectionIssues =
                SophiaLinter.runLinter(parsed, linterContext, false)?.map(
                    (linterWarning) => {
    ...
    _updatePreviewFrames() {
        if (this.props.mode === "preview") {
            const frameRef = this.frameRefs.get("all");
            safeSendNewData(frameRef, {
                type: "article-all",
            ...
            this._sections().forEach((section, i) => {
                const frameRef = this.frameRefs.get(i);
                safeSendNewData(frameRef, {
                    type: "article",
    ...
    _apiOptionsForSection(
        section: RendererProps,
        sectionIndex: number,
    ): unknown {
        const editor = this.editorRefs.get(sectionIndex);
        return {
            apiOptions: {
                ...ApiOptions.defaults,
            ...
            legacyPerseusLint: safeGetSaveWarnings(editor),
        };
    }
                            ...
                            <fieldset disabled={editingDisabled}>
                                <div className="perseus-editor-left-cell">
                                    ...
                                    <Editor
                                        ...
                                        ref={(ref) => this.setEditorRef(i, ref)}
                                    />
                                </div>
        ...
        return (
            <DeviceFramer deviceType={this.props.screen} nochrome={nochrome}>
                <IframeContentRenderer
                    ref={(ref) => this.setFrameRef(i, ref)}
    ...
    serialize(): PerseusArticle {
        if (this.props.mode === "edit") {
            return this._sections().map((_section, i) => {
                const ref = this.editorRefs.get(i);
                return (
                    safeSerialize<RendererProps>(ref) ?? {
                        content: "",
                        widgets: {},
                        images: {},
                    }
    ...
    getSaveWarnings(): ReadonlyArray<string[]> {
        ...
        return this._sections().map((_section, i) => {
            const ref = this.editorRefs.get(i);
            return safeGetSaveWarnings(ref);
        });
    }
