...
import {safeGetSaveWarnings, safeSerialize} from "./util/ref-helpers";

// eslint-disable-next-line import/no-deprecated
import type {PerseusWidget, PerseusWidgetsMap} from "@ethosengine/perseus-core";
import type {
    APIOptions,
    ChangeHandler,
    ImageUploader,
...
class Editor extends React.Component<Props, State> {
    ...
    // Map to store widget editor refs by id (replaces string refs)
    private widgetEditorRefs = new Map<string, WidgetEditor | null>();

    // eslint-disable-next-line react/sort-comp
    private setWidgetEditorRef = (id: string, ref: WidgetEditor | null) => {
        if (ref) {
            this.widgetEditorRefs.set(id, ref);
        } else {
            this.widgetEditorRefs.delete(id);
        }
    };
    ...
    getWidgetEditor(
        ...
        return (
            <WidgetEditor
                ...
                ref={(ref) => this.setWidgetEditorRef(id, ref)}
                id={id}
                key={id}
                ...
                onSubscaleMappingsChange={this.props.onSubscaleMappingsChange}
    ...
    handleDrop: (e: DragEvent) => void = (e: DragEvent) => {
        ...
        _(files)
            ...
            .each((fileAndSentinel: {file: File; sentinel: string} | null) => {
                if (fileAndSentinel) {
                    imageUploader(fileAndSentinel.file, (url: string) => {
                        // See componentDidUpdate() for how this flag is used
                        this.lastUserValue = origContent;
                        this.props.onChange({
    ...
    _maybeCopyWidgets: (e: React.SyntheticEvent<HTMLTextAreaElement>) => void =
        (e: React.SyntheticEvent<HTMLTextAreaElement>) => {
            ...
            const widgetMatches = selectedText.match(rWidgetSplit) ?? [];
            const widgetNames = _.map(widgetMatches, (syntax) => {
                const parts = Util.rWidgetParts.exec(syntax);
                return parts?.[1];
    ...
    _addWidgetToContent: (
        ...
        // Note: we have to handle null results from .match if no
        // widgets were found.
        const widgetMatches = oldContent.match(rWidgetSplit) ?? [];
        const allWidgetIds = _.map(
            widgetMatches,
            (syntax): [string, number] => {
                const match = Util.rWidgetParts.exec(syntax);
                const type = match?.[2] ?? "";
                const num = match ? +match[3] : 0;
                return [type, num];
            },
        );
    ...
    getSaveWarnings: () => any = () => {
        const widgetIds = _.intersection(
            this.widgetIds,
            Array.from(this.widgetEditorRefs.keys()),
        );
        const warnings = _(widgetIds)
            .chain()
            .map((id) => {
                const ref = this.widgetEditorRefs.get(id);
                const issues = safeGetSaveWarnings(ref);
    ...
    serialize: (options?: any) => {
        ...
        _.each(widgetIds, (id) => {
            const ref = this.widgetEditorRefs.get(id);
            const serialized = safeSerialize(ref);
            if (serialized !== null) {
                widgets[id] = serialized;
            }
        });
