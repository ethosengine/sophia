 ...
 * a list of options.
 **/
import {components} from "@ethosengine/sophia";
...
type Props = {
    ...
    disabled?: boolean;
    // An event when an option is clicked, receives the option value
...
class Option extends React.Component<Props> {
    ...
    focus(): void {
        if (this.node) {
            focusWithChromeStickyFocusBugWorkaround(this.node);
        }
    }
...
class OptionGroup extends React.Component<{
    ...
    focusedOptionRef: Option | null = null;

    componentDidMount() {
        // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
        if (this.focusedOptionRef) {
            this.focusedOptionRef.focus();
        }
    }
        ...
        return (
            ...
            >
                {React.Children.map(children, (child, index) => {
                    ...
                    const reference =
                        selected || index === 0
                            ? (node: Option) => (this.focusedOptionRef = node)
                            : null;
