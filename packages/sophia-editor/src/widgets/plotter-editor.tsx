...
import {plotterLogic, plotterPlotTypes} from "@ethosengine/perseus-core";
import {components, PlotterWidget, Util} from "@ethosengine/sophia";
...
class PlotterEditor extends React.Component<Props, State> {
    ...
    // Refs for DOM access (replacing string refs)
    private maxYRef = React.createRef<HTMLInputElement>();
    private categoriesWrapperRef = React.createRef<HTMLDivElement>();

    // eslint-disable-next-line react/sort-comp -- Helper method for ref access
    private getCategoriesInput(): HTMLInputElement | null {
        return (
            this.categoriesWrapperRef.current?.querySelector("input") ?? null
        );
    }
    ...
    changeType: (arg1: any) => void = (type) => {
        ...
        if (categories) {
            const node = this.getCategoriesInput();
            if (node) {
    ...
    setCategoriesFromScale: () => void = () => {
        ...
        const categoriesNode = this.getCategoriesInput();
        if (categoriesNode) {
            categoriesNode.value = categories.join(", ");
        }
    };
    ...
    render(): React.ReactNode {
        ...
        return (
            <div className="perseus-widget-plotter-editor">
                ...
                <div ref={this.categoriesWrapperRef}>
                    <label>
                        Categories:{" "}
                        <TextListEditor
                            layout="horizontal"
