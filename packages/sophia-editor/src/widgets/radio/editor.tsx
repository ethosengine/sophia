...
import type {FocusableHandle} from "./types";
import type {
    PerseusRadioWidgetOptions,
    PerseusRadioChoice,
    RadioDefaultWidgetOptions,
} from "@ethosengine/perseus-core";
import type {Changeable, APIOptions} from "@ethosengine/sophia";
...
class RadioEditor extends React.Component<RadioEditorProps> {
    ...
    // Map to store refs to each choice option for focus management
    private optionRefs = new Map<number, FocusableHandle | null>();

    // eslint-disable-next-line react/sort-comp
    private setOptionRef = (index: number, ref: FocusableHandle | null) => {
        if (ref) {
            this.optionRefs.set(index, ref);
        } else {
    ...
    addChoice: (arg1: boolean, arg2: any) => void = (noneOfTheAbove, e) => {
        ...
        this.props.onChange(
            ...
            () => {
                // Focus the newly added choice after render
                // Use requestAnimationFrame to ensure DOM is updated
                requestAnimationFrame(() => {
                    const ref = this.optionRefs.get(addIndex);
                    ref?.focus();
                });
            },
        );
    ...
    focus: () => boolean = () => {
        const ref = this.optionRefs.get(0);
        if (ref) {
            ref.focus();
            return true;
        }
            ...
            <div>
                ...
                {this.props.choices.map((choice, index) => (
                    <RadioOptionSettings
                        ...
                        ref={(ref) => this.setOptionRef(index, ref)}
                        index={index}
                        choice={choice}
