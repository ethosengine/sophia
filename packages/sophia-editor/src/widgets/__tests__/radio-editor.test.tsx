import {Dependencies, ApiOptions} from "@ethosengine/sophia";
...
import type {PerseusRadioChoice} from "@ethosengine/perseus-core";
...
describe("radio-editor", () => {
    ...
    describe("focus management", () => {
        ...
        it("should focus the first choice content area when focus() is called", () => {
            const editorRef = React.createRef<RadioEditor>();
            ...
            const result = editorRef.current?.focus();
            ...
            expect(document.activeElement).toBe(contentTextareas[0]);
        });

        it("should return false when focus() is called with no choices", () => {
            const editorRef = React.createRef<RadioEditor>();
        ...
        it("should focus the new choice content area when adding a choice", async () => {
            const onChangeMock = jest.fn();
            ...
            // Click "Add a choice"
            await userEvent.click(
                screen.getAllByRole("button", {name: "Add a choice"})[0],
            );

            // Get the callback that was passed to onChange
            expect(onChangeMock).toHaveBeenCalled();
            const [, callback] = onChangeMock.mock.calls[0];

            // Re-render with the new choices (simulating what the parent would do)
            const newChoices = [
                ...fourChoices,
                {content: "", id: mockIds[0], isNoneOfTheAbove: false},
            ];
            rerender(
                <RadioEditor
                    onChange={onChangeMock}
            ...
            // Execute the callback (which triggers focus)
            callback?.();

            // Run requestAnimationFrame
            jest.runAllTimers();

            // Verify the last textarea (the new one) is focused
            const contentTextareas = screen.getAllByRole("textbox", {
                name: "Content",
            });
            // eslint-disable-next-line testing-library/no-node-access
            expect(document.activeElement).toBe(
                contentTextareas[contentTextareas.length - 1],
            );
        });
    });
