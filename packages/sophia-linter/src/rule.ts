...
/**
 * Represents the type returned by String.match() or FakePatternMatch.
 * It's an array of strings with index and input properties.
 * Using RegExpMatchArray which is the proper TypeScript type for match results.
 */
type PatternMatchType = RegExpMatchArray;
    ...
    check(
        ...
        try {
            ...
            return {
                ...
                message: `Exception in rule ${this.name}: ${errorMessage}
Stack trace:
${errorStack ?? "N/A"}`,
                start: 0,
                end: content.length,
    ...
    _defaultLintFunction(
        ...
        const matchIndex = patternMatch.index ?? 0;
        return {
            message: this.message || "",
            start: matchIndex,
            end: matchIndex + patternMatch[0].length,
        };
    }
