...
export type APIOptions = Readonly<{
    ...
    /**
     * The scoring mode for assessment.
     *
     * - 'mastery': Traditional graded assessment (correct/incorrect)
     * - 'discovery': Psychometric assessment (subscale contributions)
     * - 'reflection': No scoring, just capture responses
     *
     * Consumers can register custom strategies and use their IDs here.
     * The strategy must be registered via registerScoringStrategy() before use.
     *
     * @default 'mastery' (if @ethosengine/perseus-score is imported)
     * @default 'noop' (if no scoring strategy is imported)
     */
    scoringMode?: "mastery" | "discovery" | "reflection" | string;
}>;
