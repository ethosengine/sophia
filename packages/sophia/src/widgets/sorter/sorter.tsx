import {shuffleSorter} from "@ethosengine/perseus-core";
import {linterContextDefault} from "@ethosengine/sophia-linter";
...
function getStartUserInput(
    options: SorterPublicWidgetOptions | PerseusSorterWidgetOptions,
    problemNum: number,
): PerseusSorterUserInput {
    // Handle both public options (cards) and full options (correct)
    const cards =
        "cards" in options && options.cards != null
            ? options.cards
            : "correct" in options && options.correct != null
              ? options.correct
              : [];

    const shuffled = shuffleSorter({cards}, problemNum);
