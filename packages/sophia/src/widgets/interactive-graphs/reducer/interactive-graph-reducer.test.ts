...
import type {GraphRange} from "@ethosengine/perseus-core";
...
describe("movePointInFigure error cases", () => {
    it("throws an informative error for circle graphs", () => {
        const state: InteractiveGraphState = {
            ...baseCircleGraphState,
        };
        ...
        expect(() =>
            interactiveGraphReducer(
                state,
                actions.segment.movePointInFigure(0, 0, [5, 5]),
            ),
        ).toThrow(
            "Don't use movePointInFigure for circle graphs. Use moveCenter or moveRadiusPoint instead!",
        );
    });

    it("throws an informative error for angle graphs", () => {
        const state: InteractiveGraphState = {
            ...baseAngleGraphState,
        };
        ...
        expect(() =>
            ...
            "Don't use movePointInFigure for angle graphs. Use movePoint instead!",
        );
    });
});
